...
void *  _CDECL stub_calloc_shared(size_t _nelem, size_t _size);
void    _CDECL stub_exit(int _status);
void    _CDECL stub_free(void *_ptr);
void    _CDECL stub_free_shared(void *_ptr);
char *  _CDECL stub_getenv(const char *_name);
void *  _CDECL stub_malloc(size_t _size);
void *  _CDECL stub_malloc_shared(size_t _size);
void *  _CDECL stub_realloc(void *_ptr, size_t _size);
void *  _CDECL stub_realloc_shared(void *_ptr, size_t _size);
...
LIBC_imports    _VARAPI ___imports = {
    ...
    /* <clib/attrib.h> exports */
    stub_OS_setfileattr,
    stub_OS_getcurrentdate,
    };

/* Version of the above imports table but doing memory allocation from shared
 * memory rather than process local memory.
 */

LIBC_imports    _VARAPI ___imports_shared = {
    sizeof(LIBC_imports),
...
void * _CDECL stub_calloc_shared(size_t _nelem, size_t _size)
{
    void *p = PM_mallocShared(_nelem * _size);
    if (p)
        memset(p,0,_nelem * _size);
    return p;
}
...
{
    FILESTATUS3 s;
    char    fname[PM_MAX_PATH];

    strcpy(fname, filename);

    if (DosQueryPathInfo((PSZ)fname,FIL_STANDARD,(PVOID)&s,sizeof(s)))
        return;
    s.attrFile = attrib;
    DosSetPathInfo((PSZ)fname,FIL_STANDARD,(PVOID)&s,sizeof(s),0L);
}
...
int _CDECL stub_open(const char *_path, int _oflag, unsigned _mode)
{
     ...
     */
    strcpy(path,_path);
    if (DosOpen((PSZ)path, &handle, &actiontaken, 0, FILE_NORMAL,
...
int _CDECL stub_access(const char *_path, int _amode)
{
     ...
     */
    strcpy(path,_path);
    if (DosQueryPathInfo((PSZ)path, FIL_STANDARD, &fs, sizeof(fs)) != NO_ERROR)
        return -1;
...
int _CDECL stub_unlink(const char *_path)
{
     ...
     */
    strcpy(path,_path);
    if (DosDelete((PSZ)path) != NO_ERROR)
        return -1;
    return 0;
}
...
int _CDECL stub_rename(const char *_old, const char *_new)
{
    ...
    strcpy(old,_old);
    strcpy(new,_new);
    if (DosMove((PSZ)old, (PSZ)new) != NO_ERROR)
        return -1;
    return 0;
}
...
{
#if defined(__WIN32_VXD__) || defined(__NT_DRIVER__) || defined(__SMX32__) || defined(__RTTARGET__)
    return -1;
#else
    return raise(sig);
#endif
