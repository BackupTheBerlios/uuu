...
#define CPU_HaveSSE2    0x04000000
#endif
...
CPU_haveSSE2, CPU_getProcessorName
****************************************************************************/
...
/****************************************************************************
DESCRIPTION:
Returns true if the processor supports Intel SSE2 extensions.

HEADER:
cpuinfo.h

RETURNS:
True if Intel SSE2 is available, false if not.

REMARKS:
This function determines if the processor supports the Intel SSE2 extended
instruction set.

SEE ALSO:
CPU_getProcessorType, CPU_getProcessorSpeed, CPU_haveMMX, CPU_have3DNow,
CPU_haveSSE, CPU_getProcessorName
****************************************************************************/
ibool ZAPI CPU_haveSSE2(void)
{
#ifdef  __INTEL__
    if (_CPU_haveCPUID())
        return (_CPU_getCPUIDFeatures() & CPU_HaveSSE2) != 0;
    return false;
#else
    return false;
#endif
    ...
    ulong cycles)
{
    ...
    iPriority = PM_setMaxThreadPriority();
    ...
    PM_restoreThreadPriority(iPriority);
...
****************************************************************************/
static void GetRDTSCCpuSpeed(
    ibool accurate, u64 *speed)
{
