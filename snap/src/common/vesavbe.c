/****************************************************************************
*
*                      Generic VESA VBE Interface Module
*
*  ========================================================================
*
...
static ibool        haveGeodeGX2;   /* True if we have a Geode GX2      */
...
****************************************************************************/
{
    ...
    /* Check for Geode GX2 based machines, since they have broken BIOS code.
     */
    haveGeodeGX2 = false;
    if (vgaInfo->VESAVersion >= 0x200 && (strcmp(vgaInfo->OemStringPtr,"GEODE GX2") != NULL))
        haveGeodeGX2 = true;
    ...
    /* Check for VBE 3.0 cards that report 0 for the maximum pixel clock. They
     * can't do refresh rate control, so they really are *not* VBE 3.0 compliant!
     */
    if (vgaInfo->VESAVersion >= 0x300) {
        if (!VBE_getModeInfo(0x101,&mi))
            return (state->VBEVersion = 0);
        if (mi.MaxPixelClock == 0)
            vgaInfo->VESAVersion = 0x200;
        }
...
****************************************************************************/
{
    ...
    /* Mark all modes as NonVGA for the Geode GX2, since they forgot */
    if (haveGeodeGX2)
        modeInfo->ModeAttributes |= vbeMdNonVGA;
...
****************************************************************************/
{
    ...
    /* The Geode GX2 has protected mode functions, but they stupidly try
     * to use the VGA compatible vSync stuff which does not work.
     */
    if (!state->pmInfo && state->VBEVersion >= 0x200 && !haveGeodeGX2) {
        regs.x.ax = 0x4F0A;
        regs.x.bx = 0;
        PM_int86x(0x10,&regs,&regs,&sregs);
        if (regs.x.ax != VBE_SUCCESS)
