...
the system memory cache is /not/ maintained automaticaly by SNAP Graphics, but
...
values.
 ...
 LinearSize member is the 32-bit length of the linear frame buffer memory
...
memory and system memory buffers. The PixelFormat and Palette parameters
will always be NULL for non-special surfaces, which indicate that the
buffer is an offscreen memory surface in the same pixel format as the
display mode. Special surfaces (texture memory, depth buffers, stencil
buffers etc) cannot be used as destination surfaces for regular 2D
...
typedef struct __GA_exports {
    ...
    struct GA_glCtx * (NAPIP GA_glLoadDriver)(GA_devCtx *hwCtx,ibool shared);
    ...
    ibool           (NAPIP GA_getRecommendedModes)(GA_devCtx *dc,N_int32 outputDevice,N_int32 outputHead,GA_recMode *recommendedMode,GA_recMode *recommendedMode8,GA_recMode *recommendedMode16,GA_recMode *recommendedMode24,GA_recMode *recommendedMode32);
...
are used internally by the the SNAP Graphics drivers for software stereo
...
ibool           NAPI GA_getRecommendedModes(GA_devCtx *dc,N_int32 outputDevice,N_int32 outputHead,GA_recMode *recommendedMode,GA_recMode *recommendedMode8,GA_recMode *recommendedMode16,GA_recMode *recommendedMode24,GA_recMode *recommendedMode32);

/* Functions to load and unload the GL3D device driver */

struct GA_glCtx * NAPI GA_glLoadDriver(GA_devCtx *hwCtx,ibool shared);
...
/* {secret} */
void            NAPI GA_freeLibrary(PE_MODULE *hModule);
