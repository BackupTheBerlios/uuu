...
static ibool            haveCpuSpeed = false;
#ifdef  __INTEL__
static u64              cpuSpeedHz;
    ...
    ibool accurate)
{
    if (!haveCpuSpeed) {
        __ZTimerInit();
#ifdef  __INTEL__
        CPU_getProcessorSpeedInHZ(accurate, (CPU_largeInteger*)&cpuSpeedHz);
        haveRDTSC = CPU_haveRDTSC() && (PM_gt64_32(cpuSpeedHz, 0));
        haveCpuSpeed = true;
#endif
        }
}
    ...
    LZTimerObject *tm)
{
    ...
    if (haveRDTSC) {
        ...
        PM_set64(diff64, tmCount.high, tmCount.low);
        PM_mul64_32(diff64, diff64, 1000000L);
        PM_div64(result, diff64, cpuSpeedHz);
        return PM_getlo32(result);
        }
    else
#endif
