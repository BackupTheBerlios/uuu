...
static int          _PM_int2BHooks = 0;         /* Number of Int 2B hooks       */
static PMFARPTR     _PM_oldInt2B;               /* Old int 2B vector            */
...
typedef struct {
    ...
    uint        realModeSeg;
    uint        realModeOff;
    uchar       *realModeHandler;
    long        realModeCBAddr;
    char        realModeRegs[0x32];
    void        *realModeCBPtr;
...
void _ASMAPI _PM_int2BHandler(void);
...
/* The following real mode routine is used to call a 32 bit protected
 * mode FAR function from real mode. We use this for passing up control
 * from the real mode interrupt handler to our protected mode code if
 * the interrupt happens to occur while the processor is in real mode.
 */

static uchar realModeIRQHandler[] = {
    0x00,0x00,0x00,0x00,                /* _realRMCB                            */
    0x66,0x2E,0x83,0x3E,0x00,0x00,0x00, /*      cmp     [cs:_realRMCB],0        */
    0x75,0x0E,                          /*      jnz     UsePMCB                 */
    0x66,0x50,                          /*      push    eax                     */
    0x66,0xB8,0x00,0x00,0x00,0x00,      /*      mov     eax,pm address          */
    0xCD,0x2B,                          /*      int     2Bh                     */
    0x66,0x58,                          /*      pop     eax                     */
    0xEB,0x05,                          /*      jmp     Done                    */
                                        /* UsePMCB:                             */
...
#define RM_CALLBACK_START   11
#define GLOBALS_START       22
    ...
    void *context)
{
    ...
    *((ulong*)&handle->thunk[13]) = offsetAdjust;
    *((ulong*)&handle->thunk[GLOBALS_START+0]) = (ulong)ih;
    *((ulong*)&handle->thunk[GLOBALS_START+4]) = (ulong)context;
    if (chainPrevious) {
        *((ulong*)&handle->thunk[GLOBALS_START+8]) = handle->prevHandler.off;
        *((ulong*)&handle->thunk[GLOBALS_START+12]) = handle->prevHandler.sel;
        }
    else {
        *((ulong*)&handle->thunk[GLOBALS_START+8]) = 0;
        *((ulong*)&handle->thunk[GLOBALS_START+12]) = 0;
        }
    *((ulong*)&handle->thunk[GLOBALS_START+16]) = IRQ;

    /* Set the protected mode interrupt handler */
    setISR(handle->IRQVect, (PM_intHandler)handle->thunk);

    /* Install a real mode interrupt handler as well. DOS4G/W and compatible
     * extenders do not install auto-passup interrupts for all interrupts,
     * so we have to do this manually. CauseWay does, but this will work fine
     * with CauseWay also.
     */
    if ((handle->realModeHandler = PM_allocRealSeg(sizeof(realModeIRQHandler),
            &handle->realModeSeg,&handle->realModeOff)) == NULL) {
        free(handle);
        return NULL;
        }
    handle->realModeCBPtr = &handle->thunk[RM_CALLBACK_START];
    handle->realModeCBAddr = NULL;
    if (!_DPMI_allocateCallback(handle->realModeCBPtr, handle->realModeRegs, &handle->realModeCBAddr)) {
        /* DOS extender does not support DPMI callbacks. This is probably
         * DOS4G/W, so in this case we try the hack method of reflecting the
         * interrupts via IRQ 0x2B which is always an auto-passup interrupt
         * and generally unused. This will *NOT* work with CauseWay and PMODE/W!
         */
        handle->realModeCBAddr = 0;
        if (_PM_int2BHooks++ == 0) {
            PM_getPMvect(0x2B,&_PM_oldInt2B);
            PM_setPMvect(0x2B,_PM_int2BHandler);
            }
        }
    *((ulong*)&realModeIRQHandler[0]) = handle->realModeCBAddr;
    *((ulong*)&realModeIRQHandler[17]) = (ulong)handle->thunk;
    memcpy(handle->realModeHandler,realModeIRQHandler,sizeof(realModeIRQHandler));
    _PM_setRMvect(handle->IRQVect,((long)handle->realModeSeg << 16) | (handle->realModeOff+4));
    ...
    PM_IRQHandle irqHandle)
{
    ...
    if (_PM_oldInt2B.sel) {
        if (--_PM_int2BHooks == 0)
            PM_restorePMvect(0x2B,_PM_oldInt2B);
        }

    /* Finally free the thunks */
    if (handle->realModeCBAddr)
        _DPMI_freeCallback(handle->realModeCBAddr);
    PM_freeRealSeg(handle->realModeHandler);
    PM_free(handle);
}
