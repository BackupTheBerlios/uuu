    ...
    for (m = dc->AvailableModes; *m != 0xFFFF; m++) {
        ...
        for (i = 0; i < modes.m.numModes; i++) {
            if (mi.Attributes & gaIsTextMode) {
                ...
                if (modes.m.modeList[i].xRes == xRes
                    && modes.m.modeList[i].yRes == yRes
                    && modes.m.modeList[i].bits == mi.BitsPerPixel) {
                    printf("  %4d x %4d Text @ ",
                        modes.m.modeList[i].xRes,
                        modes.m.modeList[i].yRes);
            ...
            else if (modes.m.modeList[i].xRes == mi.XResolution
                && modes.m.modeList[i].yRes == mi.YResolution
                && modes.m.modeList[i].bits == mi.BitsPerPixel) {
                printf("  %4d x %4d x %2d @ ",
                    modes.m.modeList[i].xRes,
                    modes.m.modeList[i].yRes,
                    modes.m.modeList[i].bits);
                if (mi.RefreshRateList[0] == -1) {
                    printf("Default Refresh\n");
                    }
                else {
