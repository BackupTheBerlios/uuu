...
{
    pPDB = _PM_mapPhysicalToLinear(_PM_getPDB(),PAGE_LIMIT,true);
    if (!pPDB)
    ...
    if (pPDB) {
        ...
        for (iPDB = startPDB; iPDB <= endPDB; iPDB++) {
            ...
            if (!(pPDB[iPDB] & PAGE_FLAGS_4MB)) {
                ...
                pPageTable = _PM_mapPhysicalToLinear(pageTable,PAGE_LIMIT,true);
    ...
    ibool below16M)
{
    ...
    return AllocMemBlock(size,NonPagedPoolCacheAligned,contiguous ? &paOurAddress : NULL);
}
