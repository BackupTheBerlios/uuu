...
static memshared    shared[MAX_MEMORY_SHARED] = {0};
    ...
    long size)
{
    ...
    /* Clear the memory table first before doing anything */
    if (firsttime) {
...
/****************************************************************************
REMARKS:
IOPL is always enabled for VxDs.
****************************************************************************/
...
ulong PMAPI PM_setMaxThreadPriority(void)
{
    ...
    DWORD           oldPriority,oldDecay,oldInversion;

    Get_Thread_Win32_Pri(hThread,&oldPriority,&oldDecay,&oldInversion);
    Set_Thread_Win32_Pri(hThread,15);
    return oldPriority;
}
...
{
    THREADHANDLE hThread = Get_Cur_Thread_Handle();
    Set_Thread_Win32_Pri(hThread,oldPriority);
}
...
/****************************************************************************
REMARKS:
Determine if the DOS app calling into the VxD is running in a window
****************************************************************************/
ibool PMAPI PM_runningInAWindow(void)
{
    VMHANDLE    hCRTC,hMem;

    VDD_Get_VM_Info(&hCRTC,&hMem);
    return (hCRTC == Get_Sys_VM_Handle());
}
