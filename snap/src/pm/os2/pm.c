...
#define SHAREDSEM_CHECK   ((PSZ)"\\SEM32\\SDD\\DAEMON")

#define DAEMON_NAME "SDDDAEMN.EXE"

/* Ordinals for SMP APIs (not even in the 4.5 Toolkit for some reason) */
#define ORD_DOS32GETPROCESSORSTATUS  447
#define ORD_DOS32SETPROCESSORSTATUS  448
#define ORD_DOS32QUERYTHREADAFFINITY 563
#define ORD_DOS32SETTHREADAFFINITY   564

/* #defines and typedefs from the 4.5 Toolkit in case we don't have them */
...
/* Pointers to dynamically imported APIs that may or may not be available.  */
/* VAC++ is picky and so is Watcom C 11 - only in a different way           */
#ifdef __IBMC__
APIRET (* APIENTRY pfnDosQueryThreadAffinity)(ULONG scope, PMPAFFINITY pAffinity);
APIRET (* APIENTRY pfnDosSetThreadAffinity)(PMPAFFINITY pAffinity);
APIRET (* APIENTRY pfnDosGetProcessorStatus)(ULONG ProcNum, PULONG pStatus);
APIRET (* APIENTRY pfnDosSetProcessorStatus)(ULONG ProcNum, ULONG pStatus);
APIRET (* APIENTRY pfnNet32ConfigGet2)(PSZ pszServer, PSZ pszReserved, PSZ pszComponent,
    PSZ pszParameter, PBYTE pbBuffer, ULONG ulBuffer, PULONG pulParmlen);
...
void (_ASMAPIP PM_EnableSNAPAccess)(void) = NULL;
void (_ASMAPIP PM_DisableSNAPAccess)(void) = NULL;
...
typedef struct _SESWITCHREC {
    ...
    PM_suspendApp_cb Callback;      /* Save/restore context callback        */
...
void PMAPI PM_init(void)
{
    if (!lowMem && !inPMInit) {
         ...
         * (some other program using the PM lib already started it).
         */
        if (DosOpenEventSem(SHAREDSEM_CHECK, &hevDaemon) != NO_ERROR) {
            if (DosWaitEventSem(hevDaemon, 1) != NO_ERROR) {
                /* Create shared event semaphore */
                if (DosCreateEventSem(SHAREDSEM_CHECK, &hevDaemon, DC_SEM_SHARED, FALSE) == NO_ERROR) {
                    ...
                    if (DosExecPgm(buf, sizeof(buf), EXEC_BACKGROUND, (PSZ)DAEMON_NAME,
                        ...
                        if (DosWaitEventSem(hevDaemon, 2000) == NO_ERROR) {
                            /* We don't really care if it runs or not at this point
                             * as all the functionality in SDDDAEMN is non-critical.
                             */
                            }
                        }
                    }
...
with any GRADD driver, including SNAP for OS/2.
****************************************************************************/
static ibool InitInt10(void)
{
    ...
    if (haveInt10 == -1) {
        ...
        if (DosScanEnv("SNAP_FORCE_BIOSEMU", &env) == 0)
            return haveInt10 = false;
        ...
        if (hModGENPMI || hModSDDPMI) {
            if (DosLoadModule((PSZ)buf,sizeof(buf),(PSZ)"VIDEOPMI.DLL",&hModVideoPMI) == 0) {
                ...
                if (DosQueryProcAddr(hModSDDPMI,0,(PSZ)"_SDDPMI_enableSNAPAccess",(void*)&PM_EnableSNAPAccess) != 0)
                    PM_fatalError("Unable to get EnableSNAPAccess entry point!");
                if (DosQueryProcAddr(hModSDDPMI,0,(PSZ)"_SDDPMI_disableSNAPAccess",(void*)&PM_DisableSNAPAccess) != 0)
                    PM_fatalError("Unable to get DisableSNAPAccess entry point!");
                strcpy(path,"X:\\OS2\\SVGADATA.PMI");
                path[0] = PM_getBootDrive();
                if (!hModSDDPMI && PM_VIDEOPMI32Request(&Adapter,PMIREQUEST_LOADPMIFILE,path,NULL) != 0) {
             ...
             */
            useVPMI = false;
             /* Try to open shared semaphore to see if our daemon is up. If it isn't,
              * something along the line must have failed and we can't use it. The
              * daemon should have been started from PM_init.
              */
            if (DosOpenEventSem(SHAREDSEM_CHECK, &hevDaemon) == NO_ERROR) {
                if (DosWaitEventSem(hevDaemon, 1) == NO_ERROR) {
...
const char * PMAPI PM_getSNAPPath(void)
{
    ...
    if (getenv("SNAP_PATH") != NULL)
        return getenv("SNAP_PATH");
...
const char * PMAPI PM_getMachineName(void)
{
    ...
    if (!DosLoadModule((PSZ)name, sizeof(name), (PSZ)"NETAPI32", &hmodNET32)) {
        if (!DosQueryProcAddr(hmodNET32, 0, (PSZ)"Net32ConfigGet2",
                          (PFN*)&pfnNet32ConfigGet2)) {
            rc = pfnNet32ConfigGet2(NULL, NULL, (PSZ)"requester", (PSZ)"computername",
                (PSZ)name, sizeof(name), &ulLen);
            }
        DosFreeModule(hmodNET32);
        }
    if (!rc)
        return name;
...
Needs to run on each CPU of a SMP system (or on the one
CPU of a UNI system). Does not make any assumptions about OS version and
dynamically queries for all needed APIs.
Note that this will only work on WSeB level kernels, not WSA SMP or earlier
as those kernels didn't support the affinity APIs.
****************************************************************************/
int PMAPI PM_enableWriteCombine(
    ...
    uint type)
{
    ULONG       ulCPU, ulNumCPUs = 1;
    APIRET      rc;
    HMODULE     hmodDOS;
    ULONG       aulCPUStatus[64];
    MPAFFINITY  afnSystem, afnThread;

    DosQuerySysInfo(QSV_NUMPROCESSORS, QSV_NUMPROCESSORS, &ulNumCPUs, sizeof(ulNumCPUs));
    DosQueryModuleHandle((PSZ)"DOSCALLS", &hmodDOS);
    rc = DosQueryProcAddr(hmodDOS, ORD_DOS32QUERYTHREADAFFINITY,
                          NULL, (PFN*)&pfnDosQueryThreadAffinity);

    /* See if we have everything we need - we're on SMP _and_ we have working
     * affinity APIs. We also need to check if it's safe to use the SMP APIs
     * (they won't work during bootup!).
     */
    if (!isSMPSafe || ulNumCPUs == 1 || rc != NO_ERROR ||
        pfnDosQueryThreadAffinity(AFNTY_SYSTEM, &afnSystem) != NO_ERROR) {
        rc = MTRR_enableWriteCombine(base, size, type);
        }
    else {
        /* If DosQueryThreadAffinity was present, the other APIs will be too */
        DosQueryProcAddr(hmodDOS, ORD_DOS32SETTHREADAFFINITY,
            NULL, (PFN*)&pfnDosSetThreadAffinity);
        DosQueryProcAddr(hmodDOS, ORD_DOS32GETPROCESSORSTATUS,
            NULL, (PFN*)&pfnDosGetProcessorStatus);
        DosQueryProcAddr(hmodDOS, ORD_DOS32SETPROCESSORSTATUS,
            NULL, (PFN*)&pfnDosSetProcessorStatus);

        /* Enable all CPUs. They are guaranteed to be numbered from 1 to n */
        for (ulCPU = 1; ulCPU < ulNumCPUs + 1; ulCPU++) {
            pfnDosGetProcessorStatus(ulCPU, &aulCPUStatus[ulCPU - 1]);
            pfnDosSetProcessorStatus(ulCPU, 1);
            }

        /* Process first 32 CPUs */
        afnThread.mask[1] = 0;
        for (ulCPU = 0; ulCPU < 32 && ulCPU < ulNumCPUs; ulCPU++) {
            afnThread.mask[0] = 1 << ulCPU;
            if ((afnThread.mask[0] & afnSystem.mask[0]) && aulCPUStatus[ulCPU]) {
                pfnDosSetThreadAffinity(&afnThread);
                rc = MTRR_enableWriteCombine(base, size, type);
                }
            }

        /* Process other 32 CPUs */
        afnThread.mask[0] = 0;
        for (ulCPU = 0; ulCPU < 32  && (ulCPU + 32 < ulNumCPUs); ulCPU++) {
            afnThread.mask[1] = 1 << ulCPU;
            if ((afnThread.mask[1] & afnSystem.mask[1]) && aulCPUStatus[ulCPU]) {
                pfnDosSetThreadAffinity(&afnThread);
                rc = MTRR_enableWriteCombine(base, size, type);
                }
            }

        /* Reset the thread affinity and processor states to original values */
        pfnDosSetThreadAffinity(&afnSystem);
        for (ulCPU = 1; ulCPU < ulNumCPUs + 1; ulCPU++)
            pfnDosSetProcessorStatus(ulCPU, aulCPUStatus[ulCPU - 1]);
        }

    /* We assume that on SMP, all CPUs either fail or don't fail. Having
     * some but not all CPUs fail is highly unlikely.
     */
    return rc;
}
...
/****************************************************************************
REMARKS:
Increase the thread priority to maximum, if possible. Return current priority.
****************************************************************************/
ulong PMAPI PM_setMaxThreadPriority(void)
{
    ...
    ulPriority = ptib->tib_ptib2->tib2_ulpri;
...
void PMAPI PM_restoreThreadPriority(
    ulong oldPriority)
{
    DosSetPriority(PRTYS_THREAD, (oldPriority >> 8) & 0xFF, oldPriority & 0xFF, 0);
}
...
ibool PMAPI PM_isSDDActive(void)
{
    ...
    return (DosQueryModuleHandle((PSZ)"SDDPMI.DLL", &hMod) == NO_ERROR);
}

/****************************************************************************
REMARKS:
Sets the 'SMP safe' variable. SDDGRADD watches for the event signifying this
and will set the variable to true when appropriate.
****************************************************************************/
void PMAPI PM_setSMPSafe(ibool safe)
{
    isSMPSafe = safe;
}
...
REMARKS:
This function locks access to the internal SNAP device driver code. On OS/2,
    ...
    int subsystem)
{
    ...
    if (lockCount) {
        lockCount--;
        if (!lockCount) {
            if (PM_DisableSNAPAccess != NULL)
                PM_DisableSNAPAccess();
            }
        }
    else
