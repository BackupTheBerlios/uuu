...
; Clear priority interrupt controller
...
;       quickly as possible.
;
; NOTE: Do *NOT* change the preamble code in the two entry points that
;       jump past the local variable block, without also changing the C
;       code offsets that are hard coded!!!
;----------------------------------------------------------------------------
...
_CContext       dd      0               ; Pointer to C interrupt handler context
...
; Entry point when call from real mode via DPMI callback. We get called
; with DS:ESI pointing to the real mode stack at time of call, so we
; adjust the return stack so that when we do an iret, we will end up
; at the next instruction in real mode following the call up to
; protected mode. Then the rest of the interrupt handling code can
; remain the same as when called from protected mode directly.

__IRQEntryFromRealMode:
        push    eax
        mov     ax,[ds:esi]
...
; Send an EOI to the PIC. Originally I did this only if we didn't chain
; to the old handler, but for some reason I had problems with this
; when chainging such that the old handler would sometimes not
; properly handle the EOI. Hence we always do it here for now.
...
@@ChainOldHandler:
        ...
        pop     ds
        pop     ebx
        jmp     far dword [cs:_PrevIRQ] ; Chain to previous IRQ handler
...
; _PM_int2BHandler
;----------------------------------------------------------------------------
; Interrupt handler for interrupt 2Bh which we use to pass up real mode
; interrupts in DOS4G/W.
;----------------------------------------------------------------------------
cprocfar    _PM_int2BHandler

        pushfd                  ; Setup for IRET return frame
        push    cs              ; Push code selector for return address
        call    eax             ; Call protected mode function (FAR)
        iret                    ; Return from interrupt
...
cprocfar    _PM_mousePMCB
        ...
        pushad
        mov     eax,[es:edi+1Ch]    ; Load register values from real mode
        mov     ebx,[es:edi+10h]
        mov     ecx,[es:edi+18h]
        mov     edx,[es:edi+14h]
        mov     esi,[es:edi+04h]
