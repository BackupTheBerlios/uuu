...
/* {secret} */
uchar   PCIAPI _BIOS32_service(ulong service,ulong function,ulong *physBase,ulong *length,ulong *serviceOffset,PCIBIOS_entry entry);
/* {secret} */
ushort  PCIAPI _PCIBIOS_isPresent(ulong i_eax,ulong *o_edx,ushort *o_ax,uchar *o_cl,PCIBIOS_entry entry);
/* {secret} */
ulong   PCIAPI _PCIBIOS_service(ulong r_eax,ulong r_ebx,ulong r_edi,ulong r_ecx,PCIBIOS_entry entry);
/* {secret} */
int     PCIAPI _PCIBIOS_getRouting(PCIRoutingOptionsBuffer *buf,PCIBIOS_entry entry);
/* {secret} */
ibool   PCIAPI _PCIBIOS_setIRQ(int busDev,int intPin,int IRQ,PCIBIOS_entry entry);
/* {secret} */
ulong   PCIAPI _PCIBIOS_specialCycle(int bus,ulong data,PCIBIOS_entry entry);
/* {secret} */
...
{secret}
****************************************************************************/
ulong PCIAPI PCI_getBIOSEntry(void)
{
    return PCIPhysEntry;
}
    ...
    while (numFound--) {
        ...
        if (pci->VendorID == info->VendorID && pci->DeviceID == info->DeviceID) {
            if ((pci->HeaderType & (~PCI_multiFunctionType)) == PCI_deviceType) {
                if (memcmp(&pci->u.type0.BaseAddress10,&info->u.type0.BaseAddress10,
