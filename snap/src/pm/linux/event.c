...
****************************************************************************/
#define	ENTER_CRITICAL_SECTION()    pthread_mutex_lock(&event_mutex)
#define	LEAVE_CRITICAL_SECTION()    pthread_mutex_unlock(&event_mutex)
...
void EVT_exit(void)
{
    pthread_cancel(tid_eventThread);
    _PM_restore_kb_mode();
    ShutdownMice () ;
    if (joystick0_fd) {
        close(joystick0_fd);
