...
*               the SciTech SNAP Graphics API functions.
*
****************************************************************************/
...
{
l	printf("DDCTest - SciTech SNAP Graphics DDC reading Program\n");
	...
	char *argv[])
{
    ...
    dc = LoadDriver(0);
	if (!dc) {
        printf("Unable to load SNAP driver!\n");
        return -1;
        }
	...
	printf("Doing DDC for %s %s (device %d):\n\n",
