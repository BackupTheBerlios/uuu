    ...
    doTestType doTest)
{
    ...
    if (0 <= choice && choice < maxmenu) {
        ...
        if (!(mi.Attributes & gaIsTextMode) && (mi.Attributes & gaHaveLinearBuffer))
            mode |= gaLinearBuffer;
        if (!doTest(gc,gl,xRes,yRes,bitsPerPixel,mode,refreshRate,&crtc,planeMask)) {
            GC_printf(gc,"\n");
    ...
    doTestType doTest)
{
    ...
    for (;;) {
        ...
        if (doChoice(gc,NULL,menu,maxmenu,0,0xFFFFFFFF,doTest))
            break;
        }
}
    ...
    doTestType doTest)
{
    ...
    for (;;) {
        ...
        if (doChoice(gc,NULL,menu,maxmenu,4,0xFFFFFFFF,doTest))
            break;
        }
}
    ...
    doTestType doTest)
{
    ...
    for (;;) {
        ...
        if (doChoice(gc,gl,menu,maxmenu,8,planeMask,doTest))
            break;
        }
}
...
#endif
        maxmenu = addCustomMode(gc,menu,maxmenu);
        if (doChoice(gc,gl,menu,maxmenu,numbits,planeMask,doTest))
            break;
        }
}
    ...
    GC_devCtx *gc)
{
    ...
    for (;;) {
        ...
        GC_printf(gc,"Version:      %d.%d with %d Kb memory\n",
        ...
        GC_printf(gc,". Multi Buffering:        %-3s     ", gc->dc->Attributes & gaHaveDisplayStart ? "Yes" : "No");
    ...
    GC_devCtx *gc)
{
    ...
    if ((gl = GA_glLoadDriver(gc->dc,false)) == NULL) {
        GC_clrscr(gc);
        banner(gc);
    ...
    for (;;) {
        ...
        switch (choice) {
            ...
            case '3':   testDirectColor(gc,gl,0xFFFFFFL,doOpenGLTest);      break;
...
#ifndef LITE
        GC_printf(gc,"  [A] - VESA DDC/CI Interactive Monitor Control Tests\n");
        GC_printf(gc,"  [B] - Hardware Scrolling Test\n");
        GC_printf(gc,"  [C] - Zooming Test\n");
        GC_printf(gc,"  [D] - Stereo LC glasses Test\n");
#endif
        GC_printf(gc,"  [Q] - Quit\n\n");
        GC_printf(gc,"Choice: ");
...
#ifndef LITE
            case 'a':   DDCCITests(gc);             break;
            case 'b':   ScrollingTest(gc);          break;
            case 'c':   ZoomingTest(gc);            break;
            case 'd':   StereoTest(gc);             break;
#endif
            }
...
center.
****************************************************************************/
static void EVTAPI PollChangesHeartBeatCallback(
    void *params)
{
...
static void help(void)
{
    ...
    printf("    -n       - Force a graphics font size (1 = 8x8, 2 = 8x14, 3 = 8x6)\n");
#endif
    exit(1);
}
#endif
    ...
    char *argv[])
{
    ...
    do {
        option = getcmdopt(argc,argv,"segfplit:k:bac:n:m:xXd",&argument);
        switch (option) {
            case 's':
                softwareOnly = true;
                break;
...
int main(int argc,char *argv[])
{
    ...
    EVT_setHeartBeatCallback(PollChangesHeartBeatCallback,NULL);
    ...
    if ((primaryGC = GC_create(dc,textModeX,textModeY,fontSize)) == NULL)
        if ((primaryGC = GC_create(dc,80,25,fontSize)) == NULL) {
            PM_fatalError("Unable to find 80x25 text mode!");
            }
