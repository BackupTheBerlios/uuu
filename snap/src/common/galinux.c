...
PM_imports * NAPI GA_getSystemPMImports(void)
{
    ...
    //       drivers on Linux, even for static linked programs!
    return &_PM_imports;
}
...
/****************************************************************************
REMARKS:
Return the shared SNAP Graphics export functions if found.
****************************************************************************/
    ...
    ibool shared)
{
    ...
    if ((exp = PM_getGASharedExports()) != NULL) {
...
# warning POWERPC has something similar...
    return false;
#else
    return false;
#endif
    ...
    GA_largeInteger *value)
{
    ...
    if (haveRDTSC)
        ...
        *value = t.tv_sec*1000000 + t.tv_usec;
#else
