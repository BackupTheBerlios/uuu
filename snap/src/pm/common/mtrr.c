...
static int MTRR_haveWriteCombine(void)
{
    ...
    switch (cpuFamily) {
        ...
        case CPU_Intel:
            PM_readMSR(INTEL_cap_MSR,&config,&dummy);
            return (config & (1 << 10));
        case CPU_Cyrix:
            ...
            return 1;
        case CPU_VIA:
            /* VIA CyrixIII and later support the MTRR registers */
            return 1;
        }
    return 0;
}
    ...
    if (cpuFamily != CPU_AMD || (cpuFamily == CPU_AMD && cpuType >= CPU_AMDAthlon)) {
        switch (cpuFamily) {
            ...
            case CPU_AMD:
                ...
                PM_readMSR(INTEL_defType_MSR,&c->defTypeLo,&c->defTypeHi);
                PM_writeMSR(INTEL_defType_MSR,c->defTypeLo & 0xF300UL,c->defTypeHi);
                break;
            case CPU_Cyrix:
                /* Cyrix CPU's need IOPL access */
                c->oldIOPL = PM_setIOPL(3);
    ...
    MTRRContext *c)
{
    if (cpuFamily != CPU_AMD || (cpuFamily == CPU_AMD && cpuType >= CPU_AMDAthlon)) {
        ...
        switch (cpuFamily) {
            case CPU_Intel:
            case CPU_AMD:
                PM_writeMSR(INTEL_defType_MSR,c->defTypeLo,c->defTypeHi);
                _MTRR_restoreCR4(c->cr4Val);
                break;
            case CPU_Cyrix:
                _MTRR_setCx86(CX86_CCR3,(uchar)c->ccr3);
                PM_setIOPL(c->oldIOPL);
                break;
            }
        }
    ...
    int *type)
{
    ...
    PM_readMSR(INTEL_physMask_MSR(reg),&maskLo,&hi);
    if ((maskLo & 0x800) == 0) {
    ...
    PM_readMSR(INTEL_physBase_MSR(reg),&baseLo,&hi);
    ...
    int type)
{
    ...
    if (size == 0) {
         ...
         */
        PM_writeMSR(INTEL_physMask_MSR(reg),0,0);
        }
    else {
        PM_writeMSR(INTEL_physBase_MSR(reg),base | type,0);
        PM_writeMSR(INTEL_physMask_MSR(reg),~(size - 1) | 0x800,0);
        }
    MTRR_endUpdate(&c);
}
...
static void INTEL_disableBankedWriteCombine(void)
{
    ...
    MTRR_beginUpdate(&c);
    PM_writeMSR(INTEL_fix16K_A0000_MSR,0,0);
    MTRR_endUpdate(&c);
}
    ...
    int *type)
{
    ...
    PM_readMSR(0xC0000085, &low, &high);
    if (reg == 1)
        low = high;
    ...
    int type)
{
     ...
     */
    PM_flushTLB();
    PM_writeMSR(0xC0000085, low, high);
    MTRR_endUpdate(&c);
}
...
if the processor supports the MTRR functionality or not. This function
must be called before you call any of the other MTRR functions.
****************************************************************************/
void MTRR_init(void)
{
    int     i,cpu,ltype;
        ...
        switch (cpuFamily) {
            case CPU_Intel:
                ...
                if (cpuType < CPU_PentiumPro)
                    return;
                PM_readMSR(INTEL_cap_MSR,&eax,&edx);
                numMTRR = eax & 0xFF;
            ...
            case CPU_Cyrix:
                ...
                break;
            case CPU_VIA:
                /* VIA CyrixIII and later support P6 style MTRR registers */
                PM_readMSR(INTEL_cap_MSR,&eax,&edx);
                numMTRR = eax & 0xFF;
                getMTRR = INTEL_getMTRR;
                setMTRR = INTEL_setMTRR;
