...
static void EVTAPI _EVT_pumpMessages(void)
{
	...
	/* Since the keyboard ISR has not been installed if NO_KEYBOARD_INTERRUPT
     * has been defined, we first check for any pending keyboard events
    ...
    int mickeyY)
{
    ...
    if (mask & 0x54) {
        ps = _EVT_disableInt();
        buttonMask = 0;
        if (mask & 4)  buttonMask |= EVT_LEFTBMASK;
        if (mask & 16)  buttonMask |= EVT_RIGHTBMASK;
        if (mask & 64) buttonMask |= EVT_MIDDLEBMASK;
