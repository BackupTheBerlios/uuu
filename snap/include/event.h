...
/****************************************************************************
REMARKS:
Defines the event message masks used to extract information for keyboard
events
...
MEMBERS:
EVT_ASCIIMASK   - ASCII code of key pressed
EVT_SCANMASK    - Scan code of key pressed
EVT_COUNTMASK   - Count for KEYREPEAT's
****************************************************************************/
typedef enum {
...
EVT_ALLBMASK    - All 'primary' (left, middle, right) buttons pressed
...
typedef enum {
    ...
    EVT_BOTHBMASK   = 0x00000007, // Is this intentional ? [CJC]
...
/****************************************************************************
REMARKS:
Defines the mouse wheel event masks

HEADER:
event.h

MEMBERS:
EVT_WHEELTYPEMASK   - Wheel type (0 for vertical wheel, 1 for horizontal)
EVT_WHEELDIRMASK    - Non-zero if the wheel was rotated up (away from user)
EVT_WHEELDELTAMASK  - Number of detents wheel was rotated
****************************************************************************/
typedef enum {
    EVT_WHEELDELTAMASK = 0x0000000f,
    EVT_WHEELDIRMASK   = 0x00000010,
    EVT_WHEELTYPEMASK  = 0x00000f00
    } EVT_eventWheelMaskType;
...
#define EVT_wheelType(m)    ((m & EVT_WHEELTYPEMASK) >> 8)
#define EVT_wheelUp(m)      ((m & EVT_WHEELDIRMASK) != 0)
#define EVT_wheelDelta(m)   (m & EVT_WHEELDELTAMASK)
...
EVT_REPAINT     - Window repaint event from OS
EVT_MOUSEWHEEL  - A mouse wheel has been rotated
...
typedef enum {
    ...
    EVT_REPAINT     = 0x00000200,
    EVT_MOUSEWHEEL  = 0x00000400,
    EVT_USEREVT     = 0x00001000
    } EVT_eventType;
...
typedef enum {
    ...
    EVT_MOUSEEVT    = (EVT_MOUSEDOWN | EVT_MOUSEAUTO | EVT_MOUSEUP | EVT_MOUSEMOVE | EVT_MOUSEWHEEL),
...
/* {secret} Standard code page tables */
